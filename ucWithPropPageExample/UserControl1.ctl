...
' Friend vs Public so when compiled it is not exposed to the user
...
Public Property Let Mode(newMode As eMode)
    ...
    If Not newMode = m_Mode Then            ' only do if property changed
        If m_Mode = lvb_ImageList Then
            ...
            UserControl.PropertyPages(0) = "ppgButton"
            UserControl.PropertyPages(1) = "StandardColor"
            UserControl.PropertyPages(1) = "StandardFont"
        End If
...
Public Property Let ImageList(imgList As String)
    ...
    If Not m_Mode = lvb_ImageList Then
        ...
        Set m_ImageLists = ImageListFromControl(m_ImageListName, True) ' validate the control & get imagelist ref
...
Attribute ImageList.VB_ProcData.VB_Invoke_Property = "PropertyPage1"
    If m_Mode = lvb_ImageList Then
...
Attribute ImageID.VB_ProcData.VB_Invoke_Property = "PropertyPage1"
    If m_Mode = lvb_ImageList Then
...
Friend Function ImageListFromControl(NameOfControl As String, bShowError As Boolean) As cImageLists
    ' Friend vs Public so when compiled it is not exposed to the user
    ' it can be called from a custom property page to
    ' return a valid cImageLists class
    ...
    If m_Mode = lvb_ImageList Then
        Set ImageListFromControl = m_ImageLists
        Exit Function
    ElseIf NameOfControl = vbNullString Then
        Set ImageListFromControl = Nothing
        Exit Function
    End If
    ...
    If Right$(NameOfControl, 1) = ")" Then
        cName = Left$(NameOfControl, InStr(NameOfControl, "(") - 1)
        cIndex = Val(Mid$(NameOfControl, InStr(NameOfControl, "(") + 1))
        Set c = Parent.Controls(cName)(cIndex)
    Else
        Set c = Parent.Controls(NameOfControl)
    End If
...
Private Sub UserControl_Show()
    ...
    Else    ' must call to set the image list reference
            Set m_ImageLists = ImageListFromControl(m_ImageListName, False)
...
Friend Sub FindImageLists(ppgComboBox As ComboBox)
...
' a listing of imagelist controls for the user to select from
        ...
        Dim sItem As String, Index As Integer
        ...
        For X = 0 To ParentControls.Count - 1
            If TypeName(ParentControls(X)) = myType Then
                If ParentControls(X).Mode = lvb_ImageList Then  ' .Mode is public, no errors
                    ...
                    Set tmpImgList = tmpUC.ImageLists
                    If Err Then
                        Err.Clear
                    Else
                        sItem = ParentControls(X).Name
                        Index = ParentControls(X).Index
                        If Err Then
                            Err.Clear
                        ElseIf Index > -1 Then
                            sItem = sItem & "(" & Index & ")"
                        End If
                        ppgComboBox.AddItem sItem
                    End If
                End If
            End If
        Next
